...
simulation.fun <- function(signal, pv.cri=0.01, ave.mu.matrix, mu.matrix1, mu.matrix2,
  ...
  for (k in 1:11) {
    ...
    fit <- eBayes(fit)
    aa <- topTable(fit, coef=2, p.value=pv.cri, num=500)
    if (nrow(aa)>0) {
...
sim.results <- sapply(1:10, function(i) {
  ...
  simulation.fun(signal, pv.cri=0.01, ave.mu.matrix, mu.matrix1, mu.matrix2,
                 sigma.matrix1, sigma.matrix2, zero.impute, rhos, sizes = sizes)
})

(sim.results0504 <- sapply(sim.results[1,], function(x) x))
write.csv(sim.results0504, file = 'sim.results_3v3_1_S1.csv', row.names = T)
#write.csv(sim.results0504, file = 'sim.results_5v5_1_S1.csv', row.names = T)
...
write.csv(ROC.ave, file = 'sim.results_3v3_2_S1.csv', row.names = T)
#write.csv(ROC.ave, file = 'sim.results_5v5_2_S1.csv', row.names = T)
