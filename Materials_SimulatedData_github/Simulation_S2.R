...
simulation.fun <- function(signal, pv.cri=0.01, ave.mu.matrix, mu.matrix1, mu.matrix2,
  ...
  l2   <- as.matrix(geneall.unique[sample(1:nrow(DE.gene.unique), 500, replace = T),])
  r2   <- as.matrix(geneall.unique[sample((nrow(DE.gene.unique)+1):nrow(geneall.unique), 500, replace = T),])
...
write.csv(sim.results0504, file = 'sim.results_3v3_1_S2.csv', row.names = T)
#write.csv(sim.results0504, file = 'sim.results_5v5_1_S2.csv', row.names = T)
...
write.csv(ROC.ave, file = 'sim.results_3v3_2_S2.csv', row.names = T)
#write.csv(ROC.ave, file = 'sim.results_5v5_2_S2.csv', row.names = T)
