...
simulation.fun <- function(signal, pv.cri=0.01, ave.mu.matrix, mu.matrix1, mu.matrix2,
  ...
  if (sizes == 5) {
    ...
    s5[idx.DE[1:500,]] <- s5[idx.DE[1:500,]] + (signal/2)
    s6[idx.DE[1:500,]] <- s6[idx.DE[1:500,]] + signal
    s9[idx.DE[1:500,]] <- s9[idx.DE[1:500,]] + (signal/2)
    s10[idx.DE[1:500,]] <- s10[idx.DE[1:500,]] + signal
    s2[idx.DE[501:nrow(DE.gene.unique),]] <- s2[idx.DE[501:nrow(DE.gene.unique),]] + (signal/2)
    s5[idx.DE[501:nrow(DE.gene.unique),]] <- s5[idx.DE[501:nrow(DE.gene.unique),]] + signal
    s6[idx.DE[501:nrow(DE.gene.unique),]] <- s6[idx.DE[501:nrow(DE.gene.unique),]] + (signal/2)
    s9[idx.DE[501:nrow(DE.gene.unique),]] <- s9[idx.DE[501:nrow(DE.gene.unique),]] + signal
    s10[idx.DE[501:nrow(DE.gene.unique),]] <- s10[idx.DE[501:nrow(DE.gene.unique),]] + (signal/2)
  ...
  num.true.p.scXY <- sum(sig.pair.name.scXY %in% c(1:m.DE))
  precesion.scXY <- num.true.p.scXY/num.p.scXY
  recall.scXY <- sum(sig.pair.name.scXY %in% c(1:m.DE))/m.DE
  FalseP.scXY <- sum(sig.pair.name.scXY %in% c((m.DE+1):nrow(genepairs) ))/nrow(genepairs)
  F1.scXY <- 2 * (precesion.scXY * recall.scXY)/(precesion.scXY + recall.scXY)
  one.p.scXY <- NA

  sig.pair.name.t <- rownames(RR)[which(RR$adj.t.p < pv.cri)]
  num.p.t <- length(sig.pair.name.t)
  num.true.p.t <- sum(sig.pair.name.t %in% c(1:m.DE ))
  precesion.t <- num.true.p.t/num.p.t
  recall.t <- sum(sig.pair.name.t %in% c(1:m.DE))/m.DE
  FalseP.t <- sum(sig.pair.name.t %in% c((m.DE+1):nrow(genepairs) ))/nrow(genepairs)
  F1.t <- 2 * (precesion.t * recall.t)/(precesion.t + recall.t)
  one.p.t <- NA

  sig.pair.name.limma <- rownames(RR)[idx.limma.sig.gene.pairs]
  num.p.limma <- length(sig.pair.name.limma)
  num.true.p.limma <- sum(sig.pair.name.limma %in% c(1:m.DE ))
  precesion.limma <- num.true.p.limma/num.p.limma
  recall.limma <- sum(sig.pair.name.limma %in% c(1:m.DE))/m.DE
  FalseP.limma <- sum(sig.pair.name.limma %in% c((m.DE+1):nrow(genepairs) ))/nrow(genepairs)
  F1.limma <- 2 * (precesion.limma * recall.limma)/(precesion.limma + recall.limma)
  one.p.limma <- NA
...
zero.impute <- TRUE
#signal <- 1; sizes <- 5; seed <- 405 # for 5 samples
...
write.csv(sim.results0504, file = 'sim.results_3v3_1_S3.csv', row.names = T)
#write.csv(sim.results0504, file = 'sim.results_5v5_1_S3.csv', row.names = T)

(ROC.ave <- matrix(rowMeans( sapply(sim.results[2,], function(x) x), na.rm=T), ncol = 9))
colnames(ROC.ave) <- c("TP.scXY", "FP.scXY", "F1.scXY", "TP.t", "FP.t", "F1.t", "TP.limma", "FP.limma", "F1.limma")
write.csv(ROC.ave, file = 'sim.results_3v3_2_S3.csv', row.names = T)
#write.csv(ROC.ave, file = 'sim.results_5v5_2_S3.csv', row.names = T)
