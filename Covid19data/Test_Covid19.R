...
library(tidyr)
library(pheatmap)
lr_short <- spread(FRR3[,1:3], lr.gene.name, obs.xy.diff, fill=0)
rownames(lr_short) <- lr_short$lr.cell.name
lr_short <- lr_short[,-1]

#remove epithelial cells
epithelialind <- grep("Ciliated|Alveolar", rownames(lr_short))
lr_immune <- lr_short[-epithelialind,]
removeLR <- apply(lr_immune,2,sum)==0
lr_immune <- lr_immune[,!removeLR]

#CCL
pheatmap(lr_immune[,grep("CCL",colnames(lr_immune))],fontsize_col = 6,fontsize_row=6,cluster_cols = F)

#CXCL
pheatmap(lr_immune[,grep("CXCL",colnames(lr_immune))],fontsize_col = 6,fontsize_row=6,cluster_cols = F)

#IFNG
pheatmap(lr_immune[,grep("IFNG",colnames(lr_immune))],fontsize_col = 6,fontsize_row=6,cluster_cols = F)
